...
IsoMMovieBox::operator bool() const {
  ...
  for (const auto& track : tracks) {
    const auto stdsDscs = track.mdia->minf->stbl->stsd->dscs;
    for (const auto& dsc : stdsDscs) {
      if (FourCharStr({'C', 'R', 'A', 'W'}) == dsc.AbstractIsoMBox::boxType) {
        hasCRAW = true;
        break;
      }
    }
